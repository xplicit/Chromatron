...
{
    public static void CreateAndRegisterRoutes(IChromatronRouteProvider routeProvider, ChromatronController controller, IChromatronModelBinder routeParameterBinder, IChromatronDataTransferOptions dataTransferOptions)
    {
        ...
        var methodInfos = controller.GetType().GetMethods()
         .Where(m => m.GetCustomAttributes(typeof(ChromatronRouteAttribute), false).Length > 0)
         .ToArray();

        foreach (var methodInfo in methodInfos)
        {
            var attribute = methodInfo.GetCustomAttribute<ChromatronRouteAttribute>();
    ...
    private static ControllerRoute CreateDelegate(object instance, MethodInfo method, IChromatronModelBinder routeParameterBinder, IChromatronDataTransferOptions dataTransferOptions)
    {
        var args = method
            .GetParameters();
