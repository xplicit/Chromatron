...
{
    private readonly IChromatronModelBinder _routeParameterBinder;
    private readonly IChromatronDataTransferOptions _dataTransfers;
    ...
    public ControllerRoute(string name, dynamic del, IList<RouteArgument> argumentInfos, IChromatronModelBinder routeParameterBinder, IChromatronDataTransferOptions dataTransfers, bool isAsync, bool hasReturnValue)
    {
        Name = name;
        Delegate = del;
    ...
    public IChromatronResponse Invoke(IChromatronRequest request)
    {
        if (Delegate is null)
        {
    ...
    public async Task<IChromatronResponse> InvokeAsync(IChromatronRequest request)
    {
        if (Delegate is null)
        {
            return new ChromatronResponse()
    ...
    private IChromatronResponse CreateResponse(object? content)
    {
        IChromatronResponse? chromatronResponse = content as IChromatronResponse;
        if (chromatronResponse is null && HasReturnValue)
        {
                    ...
                    case TaskStatus.RanToCompletion:
                        {
                            ...
                            chromatronResponse = resultProperty?.GetValue(task) as IChromatronResponse;
                            if (chromatronResponse is null)
                            {
                                chromatronResponse = new ChromatronResponse
                                {
                                    Data = resultProperty?.GetValue(task)
                                };
                            }

                            chromatronResponse.Status     = (int)HttpStatusCode.OK;
                            chromatronResponse.StatusText = ResourceConstants.StatusOKText;
                            break;
                        }
                    ...
                    default:
                        {
                            ...
                            chromatronResponse.Data       = builder.ToString();
                            chromatronResponse.StatusText = statusText;
                            break;
                        }
                }
            }
        ...
        var chromatronRequest = content as IChromatronRequest;
        if (chromatronRequest is not null && 
            !string.IsNullOrEmpty(chromatronRequest.Id) && 
            string.IsNullOrEmpty(chromatronResponse.RequestId))
        {
            chromatronResponse.RequestId = chromatronRequest.Id;
        }

        return chromatronResponse;
    }

    private void SetRouteArguments(IChromatronRequest request)
    {
...
#pragma warning restore CS8601 // Possible null reference assignment.
    ...
    private static void ValidateRequest(IChromatronRequest request)
    {
        if (request is null)
        {
