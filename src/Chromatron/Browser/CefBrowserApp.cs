...
internal class CefBrowserApp : CefApp
{
    ...
    private readonly IChromatronConfiguration _config;
    private readonly IChromatronRequestSchemeProvider _requestSchemeProvider;
    private readonly ChromatronHandlersResolver _handlersResolver;
    ...
    public CefBrowserApp(IChromatronConfiguration config, IChromatronRequestSchemeProvider requestSchemeProvider, ChromatronHandlersResolver handlersResolver)
    {
        _config = config;
    ...
    protected override void OnRegisterCustomSchemes(CefSchemeRegistrar registrar)
    {
        ...
        var schemeHandlerList = _handlersResolver?.Invoke(typeof(IChromatronSchemeHandler));
        if (schemeHandlerList is not null && schemeHandlerList.Any())
        {
            foreach (var handler in schemeHandlerList)
            {
                if (handler is IChromatronSchemeHandler schemeHandler)
                {
